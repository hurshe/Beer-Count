"""
main.py — Beer Count HRC Warsaw v2
...
BG      = "#f5f2ec"; SURFACE = "#ffffff"; CARD    = "#faf9f6"
BORDER  = "#ddd8ce"; MUTED   = "#7a7265"; TEXT    = "#1a1a1a"
PREV_BG = "#f0ede6"

# ── Fonts ──────────────────────────────────────────
FT  = ("Segoe UI", 13, "bold")   # title
FL  = ("Segoe UI", 11)           # label
FS  = ("Segoe UI", 10)           # small
FM  = ("Consolas", 11)           # mono
FB  = ("Segoe UI", 11, "bold")   # bold
FBB = ("Segoe UI", 20, "bold")   # big bold
...
class App(ctk.CTk):
    ...
    def _build_header(self):
        ...
        ctk.CTkFrame(h, fg_color=GOLD_LT, height=2, corner_radius=0).pack(side="bottom", fill="x")
        ctk.CTkLabel(h, text="🍺  Beer Count", font=("Segoe UI",15,"bold"),
                     text_color=GOLD).pack(side="left", padx=(16,4))
        ctk.CTkLabel(h, text="Hard Rock Cafe Warsaw", font=FS,
                     text_color=MUTED).pack(side="left")
        ctk.CTkLabel(h, text=date.today().strftime("%A, %d.%m.%Y"),
                     font=FS, text_color=MUTED,
                     fg_color=GOLD_BG, corner_radius=10).pack(side="right", padx=16, pady=14)

    # ── Nav ───────────────────────────────────────
    def _build_nav(self):
        nav = ctk.CTkFrame(self, fg_color=SURFACE, height=42, corner_radius=0)
        ...
        for key, lbl in [("entry","📋  Wpis dnia"),("history","📅  Historia"),
            ...
            b = ctk.CTkButton(nav, text=lbl, width=155, height=40, corner_radius=0,
                              fg_color=SURFACE, text_color=MUTED, hover_color=GOLD_BG,
                              font=FL, command=lambda k=key: self.show_tab(k))
            b.pack(side="left")
            self._nav[key] = b
    ...
    def show_tab(self, key):
        for k, b in self._nav.items():
            if k == key:
                b.configure(fg_color=GOLD_BG, text_color=GOLD, font=FB)
            else:
                b.configure(fg_color=SURFACE, text_color=MUTED, font=FL)
    ...
    def _build_tabs(self):
        self._tabs = {}
        for key in ("entry","history","report","settings"):
            f = ctk.CTkScrollableFrame(self, fg_color=BG, corner_radius=0)
            self._tabs[key] = f
        self._build_entry()
        self._build_history()
        self._build_report()
    ...
    def _build_entry(self):
        ...
        ctk.CTkLabel(banner,
                     ...
                     font=FS, text_color=GREEN, wraplength=900).pack(padx=14, pady=8)

        # date card
        dc = self._card(f, "📅  Data wpisu")
        dr = ctk.CTkFrame(dc, fg_color="transparent")
        dr.pack(fill="x")
        ctk.CTkLabel(dr, text="Data:", font=FL, text_color=MUTED).pack(side="left", padx=(0,8))
        self.ev_date = ctk.StringVar(value=str(date.today()))
        ctk.CTkEntry(dr, textvariable=self.ev_date, width=145, font=FM).pack(side="left")
        for lbl, delta in [("← Wczoraj",-1),("Dzisiaj →",0)]:
            ctk.CTkButton(dr, text=lbl, width=95, height=28, font=FS,
                          fg_color=GOLD_BG, text_color=GOLD, hover_color=BORDER,
                          border_width=1, border_color=BORDER,
                          command=lambda d=delta: self.ev_date.set(
                              str(date.today()+timedelta(days=d)))).pack(side="left", padx=5)

        # kegs card
        kc = self._card(f, "🛢  Stan kegów — koniec zmiany")
        ctk.CTkLabel(kc, text="szare = START auto z poprzedniego dnia  |  żółte = dostawa  "
                              "|  Tara: 30L=11kg, 20L=7kg",
                     font=FS, text_color=MUTED).pack(anchor="w", pady=(0,8))
        ...
        # Summary card
        sc = self._card(f, "📊  Wynik dnia")
        self._sum_frame = ctk.CTkFrame(sc, fg_color="transparent")
        ...
        for status, (col, bg) in DIFF_COLORS.items():
            ...
            ctk.CTkLabel(chip,
                         ...
                         font=FS, text_color=col).pack(padx=6, pady=3)
        ...
        ctk.CTkButton(br, text="💾  Zapisz dzień", width=180, height=40,
                      font=FB, fg_color=GOLD, hover_color=GOLD_LT,
                      command=self._save_day).pack(side="left", padx=(0,10))
        ctk.CTkButton(br, text="🔄  Przelicz", width=120, height=40, font=FL,
                      fg_color=GOLD_BG, text_color=GOLD, hover_color=BORDER,
                      border_width=1, border_color=BORDER,
                      command=self._recalc).pack(side="left", padx=(0,10))
        ctk.CTkButton(br, text="🗑  Wyczyść", width=120, height=40, font=FL,
                      fg_color=SURFACE, text_color=MUTED, hover_color=BORDER,
                      border_width=1, border_color=BORDER,
                      command=self._clear).pack(side="left")

        self._kw = []  # keg widget vars
        self._pw = []  # pos widget vars
        self._cw = []  # corr widget vars
    ...
    def _build_keg_inputs(self, beers):
        ...
        self._kw = []
        COL_W = [115, 90, 75, 80, 80, 80, 85, 85]
        hdrs  = ["Piwo", "START\n(auto)", "DOSTAWA\n(kegi)", "PEŁNE",
                 "Otw.kg#1", "Otw.kg#2", "Otw.kg#3", "END (L)"]
        for ci, (h, w) in enumerate(zip(hdrs, COL_W)):
            ctk.CTkLabel(f, text=h, font=("Segoe UI",9,"bold"), text_color=GOLD,
                         width=w, anchor="center", justify="center").grid(
                row=0, column=ci, padx=2, pady=(0,4))

        for ri, beer in enumerate(beers):
            row_v = {"beer": beer}
            # beer name label
            ctk.CTkLabel(f, text=f"{beer['name']}\n({beer['keg']}L)",
                         font=FB, text_color=TEXT, width=115, anchor="w").grid(
                row=ri+1, column=0, padx=2, pady=3)
            # start (read-only)
            row_v["start_lbl"] = ctk.CTkLabel(f, text="—", font=FM,
                                               text_color=GOLD, width=90,
                                               fg_color=PREV_BG, corner_radius=5, anchor="center")
            row_v["start_lbl"].grid(row=ri+1, column=1, padx=2, pady=3)
            # delivery (yellow)
            row_v["delivery"] = ctk.StringVar()
            e = ctk.CTkEntry(f, textvariable=row_v["delivery"], width=75, height=30,
                             font=FM, fg_color=GOLD_BG)
            e.grid(row=ri+1, column=2, padx=2, pady=3)
            e.bind("<KeyRelease>", lambda _: self._recalc())
            # full_end
            row_v["full_end"] = ctk.StringVar()
            e2 = ctk.CTkEntry(f, textvariable=row_v["full_end"], width=80, height=30, font=FM)
            ...
            for j in range(3):
                ov = ctk.StringVar()
                oe = ctk.CTkEntry(f, textvariable=ov, width=85, height=30, font=FM,
                                  placeholder_text="—")
                oe.grid(row=ri+1, column=4+j, padx=2, pady=3)
                oe.bind("<KeyRelease>", lambda _: self._recalc())
                row_v["open_end"].append(ov)
            # end result label
            row_v["end_lbl"] = ctk.CTkLabel(f, text="—", font=FM, text_color=GOLD,
                                             width=85, anchor="center")
            row_v["end_lbl"].grid(row=ri+1, column=7, padx=2, pady=3)
            self._kw.append(row_v)
    ...
    def _build_pos_inputs(self, beers, sizes):
        ...
        for si, sz in enumerate(sizes):
            ctk.CTkLabel(f, text=sz["label"], font=("Segoe UI",9,"bold"),
                         text_color=GOLD, width=80).grid(row=0, column=si+1, padx=2)
        ctk.CTkLabel(f, text="Razem (L)", font=("Segoe UI",9,"bold"),
                     text_color=GOLD, width=90).grid(row=0, column=len(sizes)+1, padx=2)

        for ri, beer in enumerate(beers):
            ctk.CTkLabel(f, text=beer["name"], font=FB, width=110,
                         anchor="w").grid(row=ri+1, column=0, padx=2, pady=3)
            svars = []
            for si, sz in enumerate(sizes):
                sv = ctk.StringVar()
                e = ctk.CTkEntry(f, textvariable=sv, width=80, height=28, font=FM,
                                 placeholder_text="0")
                e.grid(row=ri+1, column=si+1, padx=2, pady=3)
                e.bind("<KeyRelease>", lambda _: self._recalc())
                svars.append({"var": sv, "liters": sz["liters"]})
            lbl = ctk.CTkLabel(f, text="0.00 L", font=FM, text_color=GOLD,
    ...
    def _build_corr_inputs(self, beers):
        ...
        for ci, h in enumerate(["Piwo","Spill (L)","Void (L)","Open Bar (L)","Razem (L)"]):
            ctk.CTkLabel(f, text=h, font=("Segoe UI",9,"bold"),
                         text_color=GOLD, width=110).grid(row=0, column=ci, padx=2)
        for ri, beer in enumerate(beers):
            ctk.CTkLabel(f, text=beer["name"], font=FB, width=110,
                         anchor="w").grid(row=ri+1, column=0, padx=2, pady=3)
            cv = {}
            for ci, field in enumerate(["spill","void_","open_bar"], 1):
                v = ctk.StringVar()
                e = ctk.CTkEntry(f, textvariable=v, width=110, height=28, font=FM,
                                 placeholder_text="0")
                e.grid(row=ri+1, column=ci, padx=2, pady=3)
                e.bind("<KeyRelease>", lambda _: self._recalc())
                cv[field] = v
            lbl = ctk.CTkLabel(f, text="−0.00 L", font=FM, text_color=RED,
                               width=110, anchor="center")
            lbl.grid(row=ri+1, column=4, padx=2, pady=3)
            cv["lbl"] = lbl; cv["name"] = beer["name"]
            self._cw.append(cv)

    def _load_prev_starts(self):
        prev = db.get_prev_day(self.ev_date.get())
        if not prev: return
        prev_kegs = {k["name"]: db.keg_end_liters(k) for k in prev.get("kegs", [])}
        for rw in self._kw:
            name = rw["beer"]["name"]
            if name in prev_kegs:
                rw["_start_l"] = prev_kegs[name]
                rw["start_lbl"].configure(text=f"{prev_kegs[name]:.2f} L")
            else:
                rw["_start_l"] = 0.0
    ...
    def _collect(self):
        ...
        for pw in self._pw:
            data["pos"].append({
                "name":  pw["name"],
                "sizes": [{"liters": sv["liters"], "qty": sv["var"].get() or "0"}
                          for sv in pw["sizes"]],
            })
        for cw in self._cw:
            data["corr"].append({
    ...
    def _render_summary(self, data):
        ...
        for i, keg in enumerate(data["kegs"]):
            ...
            ctk.CTkLabel(row, text=keg["name"], font=FB, width=100,
            ...
            for lbl, val in [
                ...
                ctk.CTkLabel(row, text=f"{lbl}: {val}",
                             font=FS, text_color=MUTED).pack(side="left", padx=8)
            chip = ctk.CTkFrame(row, fg_color=bg, corner_radius=10)
            chip.pack(side="right", padx=12, pady=6)
            ctk.CTkLabel(chip, text=f"  {DIFF_ICONS[status]} {diff:+.2f}L  ",
                         font=FB, text_color=col).pack(padx=4, pady=2)
        ...
        ctk.CTkLabel(tot, text="ŁĄCZNIE", font=("Segoe UI",12,"bold"),
                     text_color=GOLD).pack(side="left", padx=14, pady=8)
        ctk.CTkLabel(tot, text=f"POS: {sum(db.pos_liters(p) for p in data['pos']):.1f}L",
                     font=FL, text_color=MUTED).pack(side="left", padx=12)
        ctk.CTkLabel(tot, text=f"Kor: {sum(db.corr_liters(c) for c in data['corr']):.1f}L",
                     font=FL, text_color=MUTED).pack(side="left", padx=12)
        chip = ctk.CTkFrame(tot, fg_color=tbg, corner_radius=10)
        chip.pack(side="right", padx=14, pady=8)
        ctk.CTkLabel(chip, text=f"  {DIFF_ICONS[ts]} {tot_diff:+.2f}L  ",
                     font=("Segoe UI",13,"bold"), text_color=tcol).pack(padx=6, pady=3)
    ...
    def _clear(self):
        if not messagebox.askyesno("Wyczyścić?","Wyczyścić wszystkie pola?"): return
        for rw in self._kw:
        ...
        for cw in self._cw:
            for f in ["spill","void_","open_bar"]: cw[f].set("")
        self._recalc()
    ...
    def _build_history(self):
        ...
        ctk.CTkLabel(top, text="Miesiąc:", font=FL, text_color=MUTED).pack(side="left", padx=(0,8))
        self._hist_month = ctk.StringVar()
        self._hist_cb = ctk.CTkOptionMenu(top, variable=self._hist_month,
                                           values=["(brak)"], width=240,
    ...
    def _load_history(self):
        ...
        if months:
            ...
            if self._hist_month.get() not in labels: self._hist_month.set(labels[0])
        else:
            self._hist_cb.configure(values=["(brak)"]); self._hist_month.set("(brak)")
    ...
    def _render_history(self):
        ...
        if not entries:
            ctk.CTkLabel(self._hist_list, text="Brak wpisów.",
                         font=FL, text_color=MUTED).pack(pady=30)
            return
    ...
    def _hist_card(self, entry_date, data):
        ...
        card = ctk.CTkFrame(self._hist_list, fg_color=SURFACE, corner_radius=8,
                            border_width=1, border_color=BORDER)
        card.pack(fill="x", pady=4)

        # header row
        hdr = ctk.CTkFrame(card, fg_color=CARD, corner_radius=0)
        hdr.pack(fill="x")
        ctk.CTkLabel(hdr, text=self._fmt_date(entry_date),
                     font=FB).pack(side="left", padx=14, pady=8)
        if tot_del:
            ctk.CTkLabel(hdr, text=f"🚚 Dostawa: {tot_del} keg",
                         font=FS, text_color=GOLD,
                         fg_color=GOLD_BG, corner_radius=5).pack(side="left", padx=6)
        ctk.CTkLabel(hdr, text=f"POS: {tot_pos:.1f}L",
                     font=FS, text_color=MUTED).pack(side="right", padx=14)
        chip = ctk.CTkFrame(hdr, fg_color=bg, corner_radius=10)
        chip.pack(side="right", padx=6, pady=8)
        ctk.CTkLabel(chip, text=f"  {DIFF_ICONS[status]} {tot_diff:+.2f}L  ",
                     font=FB, text_color=col).pack(padx=4, pady=2)

        # body (bars + buttons + detail)
        ...
        for i, keg in enumerate(beers):
            ...
            ctk.CTkLabel(br, text=keg["name"], font=FS, width=90,
                         anchor="w").pack(side="left")
            ctk.CTkLabel(br, text=f"{DIFF_ICONS[s]} {diff:+.2f}L",
                         font=FM, text_color=c, width=90, anchor="e").pack(side="right")
        ...
        edit_var  = {"open": False, "det_open": False}

        def toggle_det():
            if edit_var["det_open"]:
                det_frame.pack_forget(); edit_var["det_open"] = False
                det_btn.configure(text="🔍 Pokaż szczegóły")
            else:
                self._build_det_table(det_frame, entry_date, data)
                det_frame.pack(fill="x"); edit_var["det_open"] = True
                det_btn.configure(text="🔍 Ukryj szczegóły")

        def open_edit():
            self._open_edit_window(entry_date, data)

        det_btn = ctk.CTkButton(btn_f, text="🔍 Pokaż szczegóły", width=160, height=30,
                                font=FS, fg_color=GOLD_BG, text_color=GOLD,
                                hover_color=BORDER, border_width=1, border_color=BORDER,
                                command=toggle_det)
        det_btn.pack(side="left", padx=(0,8))
        ctk.CTkButton(btn_f, text="✏️ Edytuj", width=110, height=30,
                      font=FS, fg_color=GOLD, hover_color=GOLD_LT,
                      command=open_edit).pack(side="left")

    def _build_det_table(self, parent, entry_date, data):
        for w in parent.winfo_children(): w.destroy()
        sizes = db.get_sizes()
        hdrs = ["Piwo","START (L)","Dostawa","Pełne END",
                "Otw.kg#1","Otw.kg#2","Otw.kg#3","END (L)"] + \
               [sz["label"] for sz in sizes] + \
               ["Spill","Void","Open Bar","RÓŻNICA"]
        # simple text table using labels in grid
        ...
        for ci, h in enumerate(hdrs):
            ctk.CTkLabel(frm, text=h, font=("Segoe UI",8,"bold"),
                ...
                row=0, column=ci, padx=4, pady=4, sticky="ew")
        ...
        for ri, keg in enumerate(data.get("kegs",[])):
            ...
            for ci, v in enumerate(vals):
                lbl = ctk.CTkLabel(frm, text=v, font=("Segoe UI",10),
                                   anchor="center",
                                   text_color=(col if ci==len(vals)-1 else TEXT))
                if ci == len(vals)-1:
                    lbl.configure(fg_color=bg, corner_radius=4)
                lbl.grid(row=ri+1, column=ci, padx=4, pady=3, sticky="ew")

    def _open_edit_window(self, entry_date, data):
        """Load the day into entry tab for editing."""
    ...
    def _build_report(self):
        ...
        self._rep_cb = ctk.CTkOptionMenu(top, variable=self._rep_month,
                                          values=["(brak)"], width=240,
                                          command=lambda _: self._render_report())
        self._rep_cb.pack(side="left", padx=(0,12))
        ctk.CTkButton(top, text="📥 Export Excel (miesiąc)", width=200, height=34,
                      font=FL, fg_color=GOLD, hover_color=GOLD_LT,
                      command=self._export_month).pack(side="left", padx=(0,8))
        ctk.CTkButton(top, text="📥 Export Excel (rok)", width=180, height=34,
                      font=FL, fg_color=GOLD_BG, text_color=GOLD,
                      hover_color=BORDER, border_width=1, border_color=BORDER,
    ...
    def _load_report(self):
        ...
        if months:
            ...
            if self._rep_month.get() not in labels: self._rep_month.set(labels[0])
        else:
            self._rep_cb.configure(values=["(brak)"]); self._rep_month.set("(brak)")
        self._render_report()
    ...
    def _render_report(self):
        ...
        tot_diff = sum(
            db.calc_diff(k, d["pos"][i] if i<len(d.get("pos",[])) else {"sizes":[]},
        ...
        for lbl, val2, col in [
            ...
            ctk.CTkLabel(kpi, text=lbl, font=FS, text_color=MUTED).pack(padx=14, pady=(10,2))
            ctk.CTkLabel(kpi, text=val2, font=FBB, text_color=col).pack(padx=14, pady=(0,10))
        ...
        for n, a in agg.items():
            ...
            row = ctk.CTkFrame(self._rep_body, fg_color=SURFACE, corner_radius=6,
                               border_width=1, border_color=BORDER)
            row.pack(fill="x", pady=2)
            ctk.CTkLabel(row, text=n, font=FB, width=100, anchor="w").pack(side="left", padx=12, pady=7)
            ctk.CTkLabel(row, text=f"POS: {a['pos']:.1f}L | Del: {a['del']} keg | {a['days']} dni",
                         font=FS, text_color=MUTED).pack(side="left", padx=8)
            chip = ctk.CTkFrame(row, fg_color=bg, corner_radius=10)
            chip.pack(side="right", padx=12, pady=7)
            ctk.CTkLabel(chip, text=f"  {DIFF_ICONS[s]} {diff:+.1f}L  ",
                         font=FB, text_color=col).pack(padx=4, pady=2)
        ...
        for entry_date, data in entries:
            d_diff = sum(
                db.calc_diff(k, data["pos"][i] if i<len(data.get("pos",[])) else {"sizes":[]},
                             data["corr"][i] if i<len(data.get("corr",[])) else {})
                for i, k in enumerate(data.get("kegs",[]))
            )
            s = db.diff_status(d_diff); col, bg = DIFF_COLORS[s]
            tr = ctk.CTkFrame(self._rep_body, fg_color="transparent")
            tr.pack(fill="x", pady=1)
            ctk.CTkLabel(tr, text=self._fmt_date(entry_date),
                         font=FS, text_color=MUTED, width=80).pack(side="left")
            ctk.CTkLabel(tr, text=f"{DIFF_ICONS[s]} {d_diff:+.2f}L",
                         font=FM, text_color=col, width=100, anchor="e").pack(side="right")
            track = ctk.CTkFrame(tr, fg_color="#eeebe4", height=8, corner_radius=4)
            track.pack(side="left", fill="x", expand=True, padx=8)

    def _export_month(self):
        val = self._rep_month.get()
        if "(brak)" in val: messagebox.showwarning("Brak","Wybierz miesiąc"); return
        month = val.split("[")[-1].rstrip("]").strip()
        p = filedialog.asksaveasfilename(defaultextension=".xlsx",
                                          filetypes=[("Excel","*.xlsx")],
                                          initialfile=f"BeerCount_{month}.xlsx")
        if not p: return
        if xl.export_month(month, p): messagebox.showinfo("OK", f"Zapisano:\n{p}")
        else: messagebox.showerror("Błąd","Brak danych")
    ...
    def _export_year(self):
        ...
        if not p: return
        if xl.export_year(year, p): messagebox.showinfo("OK", f"Zapisano:\n{p}")
        else: messagebox.showerror("Błąd",f"Brak danych dla roku {year}")
    ...
    def _build_settings(self):
        ...
        ctk.CTkButton(c1, text="+ Dodaj piwo", width=130, height=30, font=FS,
                      fg_color=GOLD_BG, text_color=GOLD, hover_color=BORDER,
                      border_width=1, border_color=BORDER,
        ...
        ctk.CTkButton(c2, text="+ Dodaj rozmiar", width=140, height=30, font=FS,
                      fg_color=GOLD_BG, text_color=GOLD, hover_color=BORDER,
                      border_width=1, border_color=BORDER,
        ...
        ctk.CTkButton(f, text="💾  Zapisz ustawienia", width=200, height=40,
                      font=FB, fg_color=GOLD, hover_color=GOLD_LT,
                      command=self._save_settings).pack(padx=18, pady=14, anchor="w")
        self._beer_rows = []
        self._size_rows = []
    ...
    def _render_beer_settings(self, beers):
        ...
        for b in beers:
            ...
            ctk.CTkEntry(row, textvariable=nv, width=180, height=30, font=FL).pack(side="left", padx=(0,8))
            ctk.CTkOptionMenu(row, variable=kv, values=["20","30","50"],
                              width=110).pack(side="left", padx=(0,6))
            ctk.CTkLabel(row, text="L keg", font=FS, text_color=MUTED).pack(side="left", padx=(0,10))
            ctk.CTkButton(row, text="✕", width=30, height=28, font=FL,
                          fg_color=RED_BG, text_color=RED, hover_color="#f5c0c0",
    ...
    def _render_size_settings(self, sizes):
        ...
        for s in sizes:
            ...
            row.pack(fill="x", pady=3)
            ctk.CTkEntry(row, textvariable=lv, width=80, height=30, font=FM).pack(side="left", padx=(0,8))
            ctk.CTkEntry(row, textvariable=lv2, width=80, height=30, font=FM).pack(side="left", padx=(0,6))
            ctk.CTkLabel(row, text="L/szt.", font=FS, text_color=MUTED).pack(side="left", padx=(0,10))
            ctk.CTkButton(row, text="✕", width=30, height=28, font=FL,
                          fg_color=RED_BG, text_color=RED, hover_color="#f5c0c0",
                          command=lambda r=row: r.destroy()).pack(side="left")
    ...
    def _card(self, parent, title):
        ...
        card = ctk.CTkFrame(outer, fg_color=SURFACE, corner_radius=8,
                            border_width=1, border_color=BORDER)
        card.pack(fill="x")
        ctk.CTkLabel(card, text=title, font=FT, text_color=GOLD).pack(
            anchor="w", padx=16, pady=(12,4))
        ctk.CTkFrame(card, fg_color=BORDER, height=1, corner_radius=0).pack(fill="x")
        inner = ctk.CTkFrame(card, fg_color="transparent")
        inner.pack(fill="x", padx=16, pady=10)
        return inner

    def _sec_label(self, parent, title):
        ctk.CTkLabel(parent, text=title, font=FT, text_color=GOLD).pack(
            anchor="w", pady=(12,4))
        ctk.CTkFrame(parent, fg_color=BORDER, height=1, corner_radius=0).pack(fill="x", pady=(0,6))

    def _fmt_date(self, d):
        try:
            dt = datetime.strptime(d, "%Y-%m-%d")
            days_pl = ["Pon","Wt","Śr","Czw","Pt","Sob","Ndz"]
            return f"{days_pl[dt.weekday()]} {dt.day:02d}.{dt.month:02d}.{dt.year}"
        except: return d

    def _fmt_month(self, m):
        MO = {"01":"Styczeń","02":"Luty","03":"Marzec","04":"Kwiecień","05":"Maj",
